...
- **`OomEdge` ≠ `Oom` and the retry semantics differ.** `Oom` = sandbox exceeded its own `memory_max_mb` (raise the ceiling or shrink the workload). `OomEdge` = system OOM killer picked this sandbox under Edge memory pressure (the workload was fine). Retry `OomEdge` by **re-creating the sandbox** so the scheduler picks again, possibly on a different Edge — re-execing on the same sandbox just hits the same Edge under the same pressure. See [docs/design/memory-limits.md §5.4](docs/design/memory-limits.md).
- **Edge VMs require cgroup v2 + Docker `cgroupfs` driver.** Set via `/etc/docker/daemon.json` with `native.cgroupdriver=cgroupfs` *before* dockerd starts; cloud-init in `deploy/terraform/gcp/startup-script-edge.sh.tftpl` does this. Without these, `arlee-edge` fails fast at startup (intentional — we refuse to silently drop the memory-limit semantics). If you fork the IaC, mirror the daemon.json write. Verify cgroup v2 on the VM with `mount | grep cgroup2`.
